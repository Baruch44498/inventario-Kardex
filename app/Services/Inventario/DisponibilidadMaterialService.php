...
class DisponibilidadMaterialService
{
     ...
     * @return Collection<int, array<string, float|bool>>
     */
    public function resumenesProductos(iterable $productoIds, ?int $ordenId = null): Collection
    {
        ...
        $inventarios = DB::table('inventarios')
            ...
            ->selectRaw('COALESCE(SUM(stock_maximo), 0) as stock_maximo')
            ->selectRaw(
            ...
            ->selectRaw(
                'SUM(CASE WHEN stock_maximo IS NULL OR stock_maximo <= stock_minimo THEN 1 ELSE 0 END) '
                    . 'as ubicaciones_sin_objetivo'
            )
            ->get()
            ->keyBy('producto_id');

        $comprasPendientes = DB::table('orden_compra_detalles as d')
        ...
        return $ids->mapWithKeys(function (int $productoId) use (
            ...
            $inventario = $inventarios->get($productoId);
            $fisico = round((float) ($inventario->stock_fisico ?? 0), 3);
            $minimo = round((float) ($inventario->stock_minimo ?? 0), 3);
            $maximo = round((float) ($inventario->stock_maximo ?? 0), 3);
            $objetivo = round((float) ($inventario->stock_objetivo ?? $minimo), 3);
            $objetivoConfigurado = $inventario !== null
                && (int) ($inventario->ubicaciones_sin_objetivo ?? 0) === 0;
            ...
            $pendienteCompra = round((float) ($comprasPendientes->get($productoId)->pendiente ?? 0), 3);
            $disponible = round($fisico - $reservado, 3);
            $disponibleProyectado = round($disponible + $pendienteCompra, 3);
            $nivelReposicion = $ordenId ? $minimo : $objetivo;
            $requiereReposicion = $disponible <= $minimo;
            $necesidad = $requiereReposicion
                ? max(0, round($nivelReposicion - $disponibleProyectado, 3))
                : 0.0;
            ...
            return [$productoId => [
                ...
                'stock_objetivo_configurado' => $objetivoConfigurado,
                'reservado' => $reservado,
    ...
    public function resumenProducto(int $productoId, ?int $ordenId = null): array
    {
        return $this->resumenesProductos([$productoId], $ordenId)->get($productoId, [
            ...
            'stock_maximo' => 0.0,
            'stock_objetivo' => 0.0,
            'stock_objetivo_configurado' => false,
            'reservado' => 0.0,
            'reservado_orden' => 0.0,
            'disponible' => 0.0,
            'pendiente_compra' => 0.0,
            'disponible_proyectado' => 0.0,
