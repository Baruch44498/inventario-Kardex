...
@section('content')
    ...
    <section class="panel supplier-invoice-lines-panel">
        ...
        <div class="table-wrap table-wrap--wide"><table class="data-table supplier-invoice-detail-table"><thead><tr><th>Producto</th><th class="text-right">Cantidad</th><th class="text-right">Base unitaria</th><th class="text-right">Base</th><th class="text-right">IGV</th><th class="text-right">Total con IGV</th><th class="text-right">Pendiente de recepción</th></tr></thead><tbody>
            @foreach ($factura->detalles as $detalle)
                <tr><td><strong>{{ $detalle->producto?->codigo }}</strong><span>{{ $detalle->descripcion }}</span></td><td class="text-right"><x-ui.quantity :value="$detalle->cantidad" /></td><td class="text-right"><x-ui.money :value="$detalle->precio_unitario" :currency="$factura->moneda" /></td><td class="text-right"><x-ui.money :value="$detalle->subtotal" :currency="$factura->moneda" /></td><td class="text-right"><x-ui.money :value="$detalle->impuesto" :currency="$factura->moneda" /></td><td class="text-right"><strong><x-ui.money :value="$detalle->total" :currency="$factura->moneda" /></strong></td><td class="text-right"><strong><x-ui.quantity :value="$detalle->cantidadPendienteRecepcion()" /></strong></td></tr>
            @endforeach
        </tbody></table></div>
    </section>

    <section class="panel supplier-invoice-receipts-panel">
        <div class="panel-heading panel-heading--split"><div><p class="eyebrow">Trazabilidad física</p><h2>Notas de Ingreso vinculadas</h2></div>@if ($puedeRegistrarIngreso && ! $factura->estaAnulada() && $factura->ordenCompra->permiteRecepcion())<a href="{{ route('notas-ingreso.create', ['motivo_ingreso' => 'COMPRA', 'orden_compra_id' => $factura->orden_compra_id, 'factura_proveedor_id' => $factura->id]) }}" class="button button--primary button--small"><x-ui.icon name="entry" :size="16" /> Registrar recepción</a>@endif</div>
        @if ($factura->notasIngreso->isNotEmpty())<div class="purchase-order-related__list">@foreach ($factura->notasIngreso as $nota)<a href="{{ route('notas-ingreso.show', $nota) }}"><strong>{{ $nota->codigo }}</strong><span>{{ $nota->fecha_ingreso?->format('d/m/Y') }} · <x-ui.quantity :value="$nota->detalles->sum('cantidad')" /> recibido</span></a>@endforeach</div>@else<div class="empty-table-state"><span class="empty-state__icon"><x-ui.icon name="entry" :size="28" /></span><strong>Factura aún sin recepción vinculada</strong><span>Cuando llegue el producto, Almacén registra la Nota de Ingreso desde este documento.</span></div>@endif
    </section>
    ...
    @elseif ($puedeAnular && $factura->notasIngreso->isEmpty())
