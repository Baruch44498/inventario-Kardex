@if ($orden)
    <section class="order-context-card output-order-context" data-note-origin-context>
        ...
        <dl class="order-context-card__facts"><div><dt>Tipo</dt><dd>{{ $orden->tipoOrden?->codigo ?? '—' }}</dd></div><div><dt>Área</dt><dd>{{ $areaRuta ?? 'GENERAL' }}</dd></div><div><dt>Apertura</dt><dd>{{ $orden->fecha_apertura?->format('d/m/Y') }}</dd></div><div><dt>Estado</dt><dd><span class="badge badge--info">{{ $orden->estado }}</span></dd></div></dl>
    </section>
@elseif ($proforma)
    <section class="order-context-card output-order-context" data-note-origin-context>
