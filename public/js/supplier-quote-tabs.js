(() => {
    ...
    const initSupplierQuoteTabs = () => {
        document.querySelectorAll('[data-supplier-quote-tabs]').forEach((workspace) => {
            if (workspace.dataset.supplierQuoteTabsReady === 'true') {
                return;
            }

            const tabs = new Map(
                Array.from(workspace.querySelectorAll('[data-supplier-quote-tab]'))
                    .map((tab) => [tab.dataset.supplierQuoteTab, tab])
            );
            const panels = new Map(
                Array.from(workspace.querySelectorAll('[data-supplier-quote-panel]'))
                    .map((panel) => [panel.dataset.supplierQuotePanel, panel])
            );
            ...
            workspace.classList.add('supplier-quote-workspace--tabbed');
            workspace.dataset.supplierQuoteTabsReady = 'true';
            activate(initialTab);
            ...
            tabs.forEach((tab, name) => {
                ...
                tab.addEventListener('keydown', (event) => {
                    ...
                    activate(orderedTabs[nextIndex].dataset.supplierQuoteTab, {
                        focus: true,
                        updateHash: true,
                    });
                });
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initSupplierQuoteTabs, { once: true });
    } else {
        initSupplierQuoteTabs();
    }
})();
